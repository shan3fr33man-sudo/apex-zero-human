...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!existing) {
      // First-time OAuth user — create org + membership
      ...
      if (orgError) {
        ...
        if (memberError) {
          ...
          console.log(`[auth/callback] Created org + membership for OAuth user ${user.email}`);
        }
      }

      // Send new OAuth users to onboarding to create their first company
